...
interface TabsListProps extends React.ComponentPropsWithoutRef<typeof TabsPrimitive.List> {
    items?: TabsListItemProps[],
    after?: React.ReactNode,
}

interface TabsListItemProps extends React.ComponentPropsWithoutRef<typeof TabsPrimitive.Trigger> {
    label: string,
            ...
            <div className="flex items-center justify-between">
                <TabsList {...tabs}>
                    {tabs.items?.map((item, index) => (
                        <TabsTrigger key={index} {...item}>{item.label}</TabsTrigger>
                    ))}
                </TabsList>
                {tabs?.after}
            </div>
